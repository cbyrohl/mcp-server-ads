...
async def test_author_network(mock_ctx, mock_httpx):
    ...
    mock_httpx.post("/v1/vis/author-network").mock(return_value=httpx.Response(200, json=fixture))
    result = await ads_network(
        bibcodes=["1905AnP...322..891E"],
        type="author",
        ctx=mock_ctx,
    )
...
async def test_paper_network(mock_ctx, mock_httpx):
    ...
    mock_httpx.post("/v1/vis/paper-network").mock(return_value=httpx.Response(200, json=fixture))
    result = await ads_network(
        bibcodes=["1905AnP...322..891E"],
        type="paper",
        ctx=mock_ctx,
